...
struct type_t *newTypeStruct() {
	return (struct type_t *) malloc(5 * SIZEOFUINT64);
}
...
struct dimension_t *newDimensionEntry() {
	return (struct dimension_t *) malloc(2 * SIZEOFUINT64);
}
struct field_t {
	struct field_t * next;
	uint64_t* name;
...
struct field_t *newFieldListEntry() {
	return (struct field_t *) malloc(4 * SIZEOFUINT64);
}
...
void initScanner() {
	...
	*(SYMBOLS + SYM_FOR) = (uint64_t) "for";    // hw5
...
struct symbol_table_t* createSymbolTableEntry(uint64_t which, uint64_t* string,
		uint64_t line, uint64_t class, uint64_t type, uint64_t value,
		uint64_t address);

struct symbol_table_t* searchSymbolTable(struct symbol_table_t* entry,
		uint64_t* string, uint64_t class);
struct symbol_table_t* getScopedSymbolTableEntry(uint64_t* string,
		uint64_t class);

uint64_t isUndefinedProcedure(struct symbol_table_t* entry);
uint64_t reportUndefinedProcedures();
struct field_t* getStructField(struct symbol_table_t* entry,
		uint64_t* fieldName);
...
uint64_t getDimMultiplier(struct symbol_table_t* entry, uint64_t dimNumb) {
	...
	while (i < (entry->numberOfDimensions - dimNumb)) {
		if ((uint64_t) dim == 0)
			return 0; //dimension doesn't exist
		//literal
		mult = mult * dim->size;
		dim = dim->next;
		i = i + 1;
...
void addStructElement(struct symbol_table_t* entry, uint64_t* name,
		uint64_t* type, uint64_t* structName, struct symbol_table_t* def,
		uint64_t offset) {
...
uint64_t encodeRFormat(uint64_t funct7, uint64_t rs2, uint64_t rs1,
...
uint64_t encodeIFormat(uint64_t immediate, uint64_t rs1, uint64_t funct3,
		uint64_t rd, uint64_t opcode);
uint64_t getImmediateIFormat(uint64_t instruction);
void decodeIFormat();

uint64_t encodeSFormat(uint64_t immediate, uint64_t rs2, uint64_t rs1,
		uint64_t funct3, uint64_t opcode);
uint64_t getImmediateSFormat(uint64_t instruction);
void decodeSFormat();

uint64_t encodeBFormat(uint64_t immediate, uint64_t rs2, uint64_t rs1,
		uint64_t funct3, uint64_t opcode);
...
uint64_t F3_SLL = 1; // 001 // hw1
uint64_t F3_SRL = 5; // 101 // hw1
...
uint64_t F7_SLL = 0;  // 0000000 // hw1
uint64_t F7_SRL = 0;  // 0000000 // hw1
...
void printInstructionCounter(uint64_t total, uint64_t counter,
		uint64_t* mnemonics);
...
void emitSLL(uint64_t rd, uint64_t rs1, uint64_t rs2); // hw1
void emitSRL(uint64_t rd, uint64_t rs1, uint64_t rs2); // hw1
...
uint64_t ic_jalr = 0;
uint64_t ic_ecall = 0;
uint64_t ic_sll = 0; // hw1
uint64_t ic_srl = 0; // hw1
...
void record_lui_addi_add_sub_mul_sltu_jal_jalr_sll_slr();
void do_lui();
void undo_lui_addi_add_sub_mul_divu_remu_sltu_ld_jal_jalr();

void print_addi();
void print_addi_before();
void print_addi_add_sub_mul_divu_remu_sltu_srl_sll_after();
void do_addi();

void print_add_sub_mul_divu_remu_sltu_sll_slr(uint64_t *mnemonics);
void print_add_sub_mul_divu_remu_sltu_srl_sll_before();
...
uint64_t printPerInstructionCounter(uint64_t total, uint64_t* counters,
		uint64_t max);
void printPerInstructionProfile(uint64_t* message, uint64_t total,
		uint64_t* counters);
...
void resetInterpreter() {
	...
	if (execute) {
		...
		calls = 0;
		callsPerProcedure = zalloc(
				maxBinaryLength / INSTRUCTIONSIZE * SIZEOFUINT64);

		iterations = 0;
		iterationsPerLoop = zalloc(
				maxBinaryLength / INSTRUCTIONSIZE * SIZEOFUINT64);

		loadsPerInstruction = zalloc(
				maxBinaryLength / INSTRUCTIONSIZE * SIZEOFUINT64);
		storesPerInstruction = zalloc(
				maxBinaryLength / INSTRUCTIONSIZE * SIZEOFUINT64);
	}
}
...
uint64_t nextContext(uint64_t* context) {
	return (uint64_t) context;
}
uint64_t prevContext(uint64_t* context) {
	return (uint64_t)(context + 1);
}
uint64_t PC(uint64_t* context) {
	return (uint64_t)(context + 2);
}
uint64_t Regs(uint64_t* context) {
	return (uint64_t)(context + 3);
}
uint64_t PT(uint64_t* context) {
	return (uint64_t)(context + 4);
}
uint64_t LoPage(uint64_t* context) {
	return (uint64_t)(context + 5);
}
uint64_t MePage(uint64_t* context) {
	return (uint64_t)(context + 6);
}
uint64_t HiPage(uint64_t* context) {
	return (uint64_t)(context + 7);
}
uint64_t ProgramBreak(uint64_t* context) {
	return (uint64_t)(context + 8);
}
uint64_t BumpPointer(uint64_t* context) {
	return (uint64_t)(context + 9);
}
uint64_t Exception(uint64_t* context) {
	return (uint64_t)(context + 10);
}
uint64_t FaultingPage(uint64_t* context) {
	return (uint64_t)(context + 11);
}
uint64_t ExitCode(uint64_t* context) {
	return (uint64_t)(context + 12);
}
uint64_t Parent(uint64_t* context) {
	return (uint64_t)(context + 13);
}
uint64_t VirtualContext(uint64_t* context) {
	return (uint64_t)(context + 14);
}
uint64_t Name(uint64_t* context) {
	return (uint64_t)(context + 15);
}

uint64_t* getNextContext(uint64_t* context) {
	return (uint64_t*) *context;
}
uint64_t* getPrevContext(uint64_t* context) {
	return (uint64_t*) *(context + 1);
}
uint64_t getPC(uint64_t* context) {
	return *(context + 2);
}
uint64_t* getRegs(uint64_t* context) {
	return (uint64_t*) *(context + 3);
}
uint64_t* getPT(uint64_t* context) {
	return (uint64_t*) *(context + 4);
}
uint64_t getLoPage(uint64_t* context) {
	return *(context + 5);
}
uint64_t getMePage(uint64_t* context) {
	return *(context + 6);
}
uint64_t getHiPage(uint64_t* context) {
	return *(context + 7);
}
uint64_t getProgramBreak(uint64_t* context) {
	return *(context + 8);
}
uint64_t getBumpPointer(uint64_t* context) {
	return *(context + 9);
}
uint64_t getException(uint64_t* context) {
	return *(context + 10);
}
uint64_t getFaultingPage(uint64_t* context) {
	return *(context + 11);
}
uint64_t getExitCode(uint64_t* context) {
	return *(context + 12);
}
uint64_t* getParent(uint64_t* context) {
	return (uint64_t*) *(context + 13);
}
uint64_t* getVirtualContext(uint64_t* context) {
	return (uint64_t*) *(context + 14);
}
uint64_t* getName(uint64_t* context) {
	return (uint64_t*) *(context + 15);
}

void setNextContext(uint64_t* context, uint64_t* next) {
	*context = (uint64_t) next;
}
void setPrevContext(uint64_t* context, uint64_t* prev) {
	*(context + 1) = (uint64_t) prev;
}
void setPC(uint64_t* context, uint64_t pc) {
	*(context + 2) = pc;
}
void setRegs(uint64_t* context, uint64_t* regs) {
	*(context + 3) = (uint64_t) regs;
}
void setPT(uint64_t* context, uint64_t* pt) {
	*(context + 4) = (uint64_t) pt;
}
void setLoPage(uint64_t* context, uint64_t loPage) {
	*(context + 5) = loPage;
}
void setMePage(uint64_t* context, uint64_t mePage) {
	*(context + 6) = mePage;
}
void setHiPage(uint64_t* context, uint64_t hiPage) {
	*(context + 7) = hiPage;
}
void setProgramBreak(uint64_t* context, uint64_t brk) {
	*(context + 8) = brk;
}
void setBumpPointer(uint64_t* context, uint64_t brk) {
	*(context + 9) = brk;
}
void setException(uint64_t* context, uint64_t exception) {
	*(context + 10) = exception;
}
void setFaultingPage(uint64_t* context, uint64_t page) {
	*(context + 11) = page;
}
void setExitCode(uint64_t* context, uint64_t code) {
	*(context + 12) = code;
}
void setParent(uint64_t* context, uint64_t* parent) {
	*(context + 13) = (uint64_t) parent;
}
void setVirtualContext(uint64_t* context, uint64_t* vctxt) {
	*(context + 14) = (uint64_t) vctxt;
}
void setName(uint64_t* context, uint64_t* name) {
	*(context + 15) = (uint64_t) name;
}
...
uint64_t getBits(uint64_t n, uint64_t i, uint64_t b) {
	...
	if (i == 0)
		...
		// shift to-be-loaded bits all the way to the ri<ght and return
...
uint64_t* storeCharacter(uint64_t* s, uint64_t i, uint64_t c) {
	...
	*(s + a) = (*(s + a) - (loadCharacter(s, i) << ((i % SIZEOFUINT64) * 8))
			+ (c << (i % SIZEOFUINT64) * 8));
...
void printFixedPointPercentage(uint64_t a, uint64_t b) {
	print(
			itoa(fixedPointPercentage(fixedPointRatio(a, b)), integer_buffer,
					10, 0, 2));
}
...
void getCharacter() {
	...
	//file open,pointer to memory where to put byte,1 byte)
...
uint64_t identifierOrKeyword() {
	if (identifierStringMatch(SYM_FOR)) // hw5
		return SYM_FOR;
			...
			if (isCharacterLetter()) {
				...
				while (isCharacterDigit()) {
					if (i >= maxIntegerLength) {
						if (integerIsSigned)
							syntaxErrorMessage(
									(uint64_t*) "signed integer out of bound");
						else
							syntaxErrorMessage(
									(uint64_t*) "integer out of bound");
				...
				if (character == CHAR_EOF) {
					syntaxErrorMessage(
							(uint64_t*) "reached end of file looking for a character literal");
				...
				if (character == CHAR_EQUAL) {
					...
					symbol = SYM_LEQ;
				} else if (character == CHAR_LT) { // hw2
					getCharacter();

					symbol = SYM_BITWISELEFTSHIFT;
				} else
					symbol = SYM_LT;
				...
				if (character == CHAR_EQUAL) {
					...
					symbol = SYM_GEQ;
				} else if (character == CHAR_GT) { // hw2
					getCharacter();

					symbol = SYM_BITWISERIGHTSHIFT;
				} else
					symbol = SYM_GT;
...
struct symbol_table_t* createSymbolTableEntry(uint64_t whichTable,
		uint64_t* string, uint64_t line, uint64_t class, uint64_t type,
		uint64_t value, uint64_t address) {
	struct symbol_table_t* newEntry;
	//hw4
	struct type_t* typeStruct;
	//hw3
	newEntry = (struct symbol_table_t*) malloc(
			3 * SIZEOFUINT64STAR + 7 * SIZEOFUINT64);
	//hw4 start
...
struct field_t* getStructField(struct symbol_table_t* structEntry,
		uint64_t* fieldName) {
	uint64_t* name;
	uint64_t* structName;
	struct type_t* type;
...
uint64_t isUndefinedProcedure(struct symbol_table_t* entry) {
	...
	if (entry->class == PROCEDURE) {
		// library procedures override declared or defined procedures
		libraryEntry = searchSymbolTable(library_symbol_table, entry->string,
				PROCEDURE);
...
struct symbol_table_t* getVariableOrBigInt(uint64_t* variableOrBigInt,
		uint64_t class) {
	struct symbol_table_t* entry;
...
void load_integer(uint64_t value) {
	...
	if (isSignedInteger(value, 12)) {
		...
		if (entry == (struct symbol_table_t*) 0) {
			...
			createSymbolTableEntry(GLOBAL_TABLE, integer, lineNumber, BIGINT,
					UINT64_T, value, -allocatedMemory);
		}
...
void load_string(uint64_t* string) {
	...
	createSymbolTableEntry(GLOBAL_TABLE, string, lineNumber, STRING,
			UINT64STAR_T, 0, -allocatedMemory);
...
uint64_t help_call_codegen(struct symbol_table_t* entry, uint64_t* procedure) {
	...
	if (entry == (struct symbol_table_t*) 0) {
		...
		createSymbolTableEntry(GLOBAL_TABLE, procedure, lineNumber, PROCEDURE,
				type, 0, binaryLength);
...
uint64_t compile_selector(uint64_t selectorNum, struct symbol_table_t* entry) {
	...
	if (arrayDeklaration) {
		getSymbol();
		if (symbol != SYM_INTEGER) {
			if (symbol != SYM_CHARACTER) {
				syntaxErrorMessage(
						(uint64_t*) "size of array dimension must be a literal of type integer or char");
...
uint64_t compile_structAccess(uint64_t* variableName) {
	...
	if (help_variable == 0) {
		...
		variableEntry = searchSymbolTable(local_symbol_table, variableName,
				VARIABLE);

		//if unfound look for global struct pointers
		if (variableEntry == (struct symbol_table_t*) 0) {
			// variableEntry = global_symbol_table;
			variableEntry = searchSymbolTable(global_symbol_table, variableName,
					VARIABLE);
		}
	} else {
		variableEntry = searchSymbolTable(local_symbol_table, variableName,
	...
	if (definitionEntry == (struct symbol_table_t*) 0) {
		...
		definitionEntry = searchSymbolTable(global_symbol_table, definitionName,
				STRUCT);
	...
	while (symbol == SYM_ARROW) {
		...
		if (definitionEntry == (struct symbol_table_t*) 0) {
			...
			definitionName = variableEntry->typeStruct->structName;
			definitionEntry = searchSymbolTable(global_symbol_table,
		...
		if (symbol == SYM_VOID) {
			...
			if (symbol == SYM_ASTERISK) {
				...
				if (symbol == SYM_SEMICOLON) {
					...
					entry = searchSymbolTable(global_symbol_table,
							variableOrProcedureName, VARIABLE);
					...
					if (entry == (struct symbol_table_t*) 0) {
						...
						entry = createSymbolTableEntry(GLOBAL_TABLE,
								variableOrProcedureName, lineNumber, VARIABLE,
								STRUCTSTAR_T, 0, -allocatedMemory);
					} else {
						// global variable already declared or defined
						printLineNumber((uint64_t*) "warning",
								currentLineNumber);
					...
					entry = createSymbolTableEntry(GLOBAL_TABLE,
							variableOrProcedureName, currentLineNumber, ARRAY,
							STRUCTSTAR_T, 0, 0);
					...
					entry = searchSymbolTable(global_symbol_table,
							variableOrProcedureName, PROCEDURE);

				} else
					syntaxErrorUnexpected();

				//set definition and type name
				structDef = searchSymbolTable(global_symbol_table, structName,
						STRUCT);
				//type record
				...
				entry = searchSymbolTable(global_symbol_table, structName,
						STRUCT);
				...
				entry = createSymbolTableEntry(GLOBAL_TABLE, structName,
						lineNumber, STRUCT, 0, 0, 0);
				...
				while (lookForType() == 0) {
					...
					if (type == STRUCTSTAR_T) {
						...
						structEntry = searchSymbolTable(global_symbol_table,
								structName, STRUCT);
					} else {
						//type
					...
					addStructElement(entry, variableName, type, structName,
							structEntry, offset);
				...
				// type identifier "["
					...
					if (entry == (struct symbol_table_t*) 0) {
						...
						createSymbolTableEntry(GLOBAL_TABLE,
								variableOrProcedureName, currentLineNumber,
...
uint64_t compile_term() {
	...
	while (isStarOrDivOrModulo()) {
		...
		if (operatorSymbol == SYM_ASTERISK)
			emitMUL(previousTemporary(), previousTemporary(),
					currentTemporary());
		else if (operatorSymbol == SYM_DIV)
			emitDIVU(previousTemporary(), previousTemporary(),
					currentTemporary());
		else if (operatorSymbol == SYM_MOD)
			emitREMU(previousTemporary(), previousTemporary(),
					currentTemporary());
		...
		if (operatorSymbol == SYM_PLUS) {
			if (ltype == UINT64STAR_T) {
				if (rtype == UINT64_T)
					...
					syntaxErrorMessage(
							(uint64_t*) "(uint64_t*) + (uint64_t*) is undefined");
			...
			emitADD(previousTemporary(), previousTemporary(),
					currentTemporary());
			...
			if (ltype == UINT64STAR_T) {
				if (rtype == UINT64_T) {
					...
					// pointer arithmetic: factor of 2^3 of integer operand
					emitLeftShiftBy(currentTemporary(), 3);
					emitSUB(previousTemporary(), previousTemporary(),
							currentTemporary());
				} else {
					// UINT64STAR_T - UINT64STAR_T
...
uint64_t compile_bitwiseShift() {
	...
	while (isShift()) {
		...
		if (operatorSymbol == SYM_BITWISELEFTSHIFT) {
			...
			//R[rd] = R[rs1] << R[rs2]
			//rd,rs1,rs2
			emitSLL(previousTemporary(), previousTemporary(),
					currentTemporary());

		} else if (operatorSymbol == SYM_BITWISERIGHTSHIFT) {
			//R[rd] = R[rs1] >> R[rs2]

			emitSRL(previousTemporary(), previousTemporary(),
					currentTemporary());
		}
...
uint64_t compile_expression() {
	...
	if (isComparison()) {
		...
		if (operatorSymbol == SYM_EQUALITY) {
			...
			emitSUB(previousTemporary(), currentTemporary(),
					previousTemporary());
			emitADDI(currentTemporary(), REG_ZR, 1);
			emitSLTU(previousTemporary(), previousTemporary(),
					currentTemporary());
			...
			// a > b iff b < a
			emitSLTU(previousTemporary(), currentTemporary(),
					previousTemporary());
...
void compile_for() {
	//																		ships
	uint64_t jumpBackToCondition;			//circle
	uint64_t jumpToStatement;			    //tree Angel
	uint64_t jumpBackToIncrement;			//tree Angel help for not endless loop
	...
	if (symbol == SYM_FOR) {
		...
		if (symbol == SYM_LPARENTHESIS) {
			...
			if (symbol == SYM_IDENTIFIER) {
				...
				if (symbol == SYM_ASSIGN) {
					...
					if (isSignedInteger(offset, 12)) {
						...
						emitADD(currentTemporary(), entry->scope,
								currentTemporary());
			...
			jumpBackToCondition = binaryLength; //circle
			...
			jumpToStatement = binaryLength; //tree Angel
			...
			emitJAL(REG_ZR, jumpBackToCondition - binaryLength); // jump to circle ships
			...
			if (symbol == SYM_RPARENTHESIS) {
				...
				if (symbol == SYM_LBRACE) {
					...
					// if the "for expression" case was true we unconditionally jump here
					fixup_relative_JFormat(jumpToStatement, binaryLength);
	...
	// 1. the RISC-V doc recommends to do so to not disturb branch prediction
	// 2. GCC also uses JAL for the unconditional back jump of a while loop
	emitJAL(REG_ZR, jumpBackToIncrement - binaryLength); // jump to circle ships
...
void compile_variable(uint64_t offset) {
	...
	if (symbol == SYM_IDENTIFIER) {
		...
		if (symbol == SYM_LBRACKET) {
			...
			entry = createSymbolTableEntry(LOCAL_TABLE, identifier, lineNumber,
					ARRAY, type, 0, 0);
			...
			if (type == STRUCTSTAR_T) {
				...
				def = searchSymbolTable(global_symbol_table, structTypeName,
						STRUCT);
		...
		createSymbolTableEntry(LOCAL_TABLE, (uint64_t*) "missing variable name",
...
void compile_procedure(uint64_t* procedure, uint64_t type) {
	...
	if (symbol == SYM_SEMICOLON) {
		...
		if (entry == (struct symbol_table_t*) 0)
			// procedure never called nor declared nor defined
			createSymbolTableEntry(GLOBAL_TABLE, procedure, lineNumber,
					PROCEDURE, type, 0, 0);
		...
		while (lookForHilf()) {
			...
			localVariables = localVariables + hilfbits; // hw3
...
void selfie_compile() {
	...
	createSymbolTableEntry(GLOBAL_TABLE, (uint64_t*) "main", 0, PROCEDURE,
			UINT64_T, 0, 0);
	...
	while (link) {
		if (numberOfRemainingArguments() == 0)
			...
			sourceFD = signExtend(open(sourceName, O_RDONLY, 0),
					SYSCALL_BITWIDTH);
...
void emitECALL() {
	emitInstruction(
			encodeIFormat(F12_ECALL, REG_ZR, F3_ECALL, REG_ZR, OP_SYSTEM));
...
void fixup_relative_BFormat(uint64_t fromAddress) {
	...
	storeInstruction(fromAddress,
			encodeBFormat(binaryLength - fromAddress, getRS2(instruction),
					getRS1(instruction), getFunct3(instruction),
...
void fixup_relative_JFormat(uint64_t fromAddress, uint64_t toAddress) {
	...
	storeInstruction(fromAddress,
			encodeJFormat(toAddress - fromAddress, getRD(instruction),
					getOpcode(instruction)));
}
...
uint64_t openWriteOnly(uint64_t* name) {
	...
	// try Mac flags
	fd = signExtend(
			open(name, MAC_O_CREAT_TRUNC_WRONLY, S_IRUSR_IWUSR_IRGRP_IROTH),
			SYSCALL_BITWIDTH);

	if (signedLessThan(fd, 0)) {
		// try Linux flags
		fd = signExtend(
				open(name, LINUX_O_CREAT_TRUNC_WRONLY,
						S_IRUSR_IWUSR_IRGRP_IROTH), SYSCALL_BITWIDTH);

		if (signedLessThan(fd, 0))
			// try Windows flags
			fd = signExtend(
					open(name, WINDOWS_O_BINARY_CREAT_TRUNC_WRONLY,
							S_IRUSR_IWUSR_IRGRP_IROTH), SYSCALL_BITWIDTH);
	}
		...
		if (parseELFHeader(ELF_header)) {
			...
			if (numberOfReadBytes == SIZEOFUINT64) {
				...
				if (binaryLength <= maxBinaryLength) {
					...
					numberOfReadBytes = signExtend(
							read(fd, binary, binaryLength), SYSCALL_BITWIDTH);
...
void emitExit() {
	createSymbolTableEntry(LIBRARY_TABLE, (uint64_t*) "exit", 0, PROCEDURE,
			VOID_T, 0, binaryLength);
...
void implementExit(uint64_t* context) {
	...
	print((uint64_t*) " and ");
	printFixedPointRatio(getBumpPointer(context) - getProgramBreak(context),
			MEGABYTE);
...
void emitRead() {
	createSymbolTableEntry(LIBRARY_TABLE, (uint64_t*) "read", 0, PROCEDURE,
			UINT64_T, 0, binaryLength);
	...
	while (size > 0) {
		if (isValidVirtualAddress(vbuffer)) {
			if (isVirtualAddressMapped(getPT(context), vbuffer)) {
				...
				actuallyRead = signExtend(read(fd, buffer, bytesToRead),
						SYSCALL_BITWIDTH);
				...
				if (debug_read) {
					...
					printHexadecimal(vbuffer, 8);
					print(
							(uint64_t*) " failed because the address is unmapped");
					println();
				}
			}
		} else {
...
void emitWrite() {
	createSymbolTableEntry(LIBRARY_TABLE, (uint64_t*) "write", 0, PROCEDURE,
			UINT64_T, 0, binaryLength);
	...
	while (size > 0) {
		if (isValidVirtualAddress(vbuffer)) {
			if (isVirtualAddressMapped(getPT(context), vbuffer)) {
				...
				actuallyWritten = signExtend(write(fd, buffer, bytesToWrite),
						SYSCALL_BITWIDTH);
...
void emitOpen() {
	createSymbolTableEntry(LIBRARY_TABLE, (uint64_t*) "open", 0, PROCEDURE,
			UINT64_T, 0, binaryLength);
			...
			if (isVirtualAddressMapped(table, vstring)) {
				...
				*(s + i) = loadPhysicalMemory(pstring);
				...
				if (debug_open) {
					print(selfieName);
					print(
							(uint64_t*) ": opening file with name at virtual address ");
...
void emitMalloc() {
	createSymbolTableEntry(LIBRARY_TABLE, (uint64_t*) "malloc", 0, PROCEDURE,
			UINT64STAR_T, 0, binaryLength);

	// on boot levels higher than zero, zalloc falls back to malloc
	// assuming that page frames are zeroed on boot level zero
	createSymbolTableEntry(LIBRARY_TABLE, (uint64_t*) "zalloc", 0, PROCEDURE,
			UINT64STAR_T, 0, binaryLength);
...
void emitSwitch() {
	createSymbolTableEntry(LIBRARY_TABLE, (uint64_t*) "hypster_switch", 0,
			PROCEDURE, UINT64STAR_T, 0, binaryLength);
...
void implementSwitch() {
	...
	doSwitch(cacheContext((uint64_t*) *(registers + REG_A0)),
			*(registers + REG_A1));
}
...
void do_lui() {
	...
	// Load constant into upper bits of word
...
//undoes the effect,go back,rd before
//x=0,x/x,you can see haw you come to div by 0
void undo_lui_addi_add_sub_mul_divu_remu_sltu_ld_jal_jalr() {
...
//print value in rd nach execution
void print_addi_add_sub_mul_divu_remu_sltu_srl_sll_after() {
	print((uint64_t*) " -> ");
	printRegisterValue(rd);
}
//initialisation rd,rs,imm
void do_addi() {
	// add immediate
...
//print before instruction execute
void print_add_sub_mul_divu_remu_sltu_srl_sll_before() {
	print((uint64_t*) ": ");
	printRegisterValue(rs1);
	print((uint64_t*) ",");
...
uint64_t do_ld() {
	uint64_t vaddr;
	uint64_t a;
//rd,imm(offset),rs(reg relative adress)
	// load double word in reg
	...
	if (redo) {
		...
		*(registers + REG_A0) = *(values + (tc % maxTraceLength));
		...
		if (record) {
			print(selfieName);
			print(
					(uint64_t*) ": context switching during recording is unsupported");
			println();
...
void decode_execute() {
	...
	if (opcode == OP_IMM) {
		...
		if (funct3 == F3_ADDI) {
			if (debug) {
				if (record) {
					record_lui_addi_add_sub_mul_sltu_jal_jalr_sll_slr();
					do_addi();
				} else if (undo)
					...
					if (execute) {
						print_addi_before();
						do_addi();
						print_addi_add_sub_mul_divu_remu_sltu_srl_sll_after();
					}
					println();
				}
			} else
			...
			if (funct7 == F7_ADD) {
				if (debug) {
					if (record) {
						...
						print_add_sub_mul_divu_remu_sltu_sll_slr(
								(uint64_t*) "add");
						if (execute) {
							print_add_sub_mul_divu_remu_sltu_srl_sll_before();
							do_add();
							print_addi_add_sub_mul_divu_remu_sltu_srl_sll_after();
						}
						println();
...
void printProfile() {
	...
	if (getTotalNumberOfInstructions() > 0) {
		...
		if (sourceLineNumber != (uint64_t*) 0)
			print(
					(uint64_t*) ": profile: total,max(ratio%)@addr(line#),2max,3max");
		else
			print((uint64_t*) ": profile: total,max(ratio%)@addr,2max,3max");
		println();

		printPerInstructionProfile((uint64_t*) ": calls:   ", calls,
				callsPerProcedure);
		printPerInstructionProfile((uint64_t*) ": loops:   ", iterations,
				iterationsPerLoop);
		printPerInstructionProfile((uint64_t*) ": loads:   ", ic_ld,
				loadsPerInstruction);
		printPerInstructionProfile((uint64_t*) ": stores:  ", ic_sd,
				storesPerInstruction);
	}
}
...
uint64_t* allocateContext(uint64_t* parent, uint64_t* vctxt, uint64_t* in) {
	...
	setMePage(context, 0);
	setHiPage(context,
			getPageOfVirtualAddress(VIRTUALMEMORYSIZE - REGISTERSIZE));
...
void saveContext(uint64_t* context) {
	...
	if (getParent(context) != MY_CONTEXT) {
		...
		storeVirtualMemory(parentTable, BumpPointer(vctxt),
				getBumpPointer(context));

		storeVirtualMemory(parentTable, Exception(vctxt),
				getException(context));
		storeVirtualMemory(parentTable, FaultingPage(vctxt),
				getFaultingPage(context));
...
void mapPage(uint64_t* context, uint64_t page, uint64_t frame) {
	...
	*(table + page) = frame;
	...
	if (page
			<= getPageOfVirtualAddress(
					getBumpPointer(context) - REGISTERSIZE)) {
...
void restoreContext(uint64_t* context) {
	...
	if (getParent(context) != MY_CONTEXT) {
		...
		setBumpPointer(context,
				loadVirtualMemory(parentTable, BumpPointer(vctxt)));

		setException(context, loadVirtualMemory(parentTable, Exception(vctxt)));
		setFaultingPage(context,
				loadVirtualMemory(parentTable, FaultingPage(vctxt)));
		...
		while (page <= me) {
			if (isVirtualAddressMapped(parentTable,
					FrameForPage(table, page))) {
				frame = loadVirtualMemory(parentTable,
						FrameForPage(table, page));

				mapPage(context, page,
						getFrameForPage(parentTable,
								getPageOfVirtualAddress(frame)));
			}
	...
	while (1) {
		...
		if (getParent(fromContext) != MY_CONTEXT) {
			...
			if (getException(fromContext) == EXCEPTION_PAGEFAULT)
				...
				mapPage(fromContext, getFaultingPage(fromContext),
						(uint64_t) palloc());
...
uint64_t selfie_run(uint64_t machine) {
	...
	up_loadArguments(currentContext, numberOfRemainingArguments(),
			remainingArguments());
...
uint64_t instanceMayBeTrue(uint64_t depth) {
	...
	while (clause < numberOfSATClauses) {
		if (clauseMayBeTrue(SATInstance + clause * 2 * numberOfSATVariables,
				depth))
			clause = clause + 1;
		else
...
void selfie_printDimacs() {
	...
	while (clause < numberOfSATClauses) {
		...
		while (variable < numberOfSATVariables) {
			if (*(SATInstance + clause * 2 * numberOfSATVariables + 2 * variable)
					== TRUE) {
				printInteger(variable + 1);
				print((uint64_t*) " ");
			} else if (*(SATInstance + clause * 2 * numberOfSATVariables
					+ 2 * variable + 1) == TRUE) {
				printInteger(-(variable + 1));
				print((uint64_t*) " ");
			}
	...
	while (1) {
		...
		if (symbol == SYM_INTEGER) {
			if (literal == 0) {
				...
				syntaxErrorMessage(
						(uint64_t*) "clause exceeds declared number of variables");
			...
			if (not)
				*(SATInstance + clause * 2 * numberOfSATVariables + 2 * literal
						+ 1) = TRUE;
			else
				*(SATInstance + clause * 2 * numberOfSATVariables + 2 * literal) =
						TRUE;
...
void dimacs_getInstance() {
	...
	while (clauses < numberOfSATClauses)
		if (symbol != SYM_EOF) {
			...
			syntaxErrorMessage(
					(uint64_t*) "instance has fewer clauses than declared");
	...
	if (symbol != SYM_EOF) {
		syntaxErrorMessage(
				(uint64_t*) "instance has more clauses than declared");
...
void selfie_loadDimacs() {
	...
	SATInstance = (uint64_t*) smalloc(
			numberOfSATClauses * 2 * numberOfSATVariables * SIZEOFUINT64);
...
void printUsage() {
	...
	print(
			(uint64_t*) "selfie { -c { source } | -o binary | -s assembly | -l binary | -sat dimacs } ");
...
uint64_t main(uint64_t argc, uint64_t* argv) {
	...
	print((uint64_t*) "This is Mostafa Masud`s Selfie"); // hw0
	println();
	...
	// a[0][0][1] = 4;
	// a[0][0][2] = 5;
	// println();print((uint64_t*)"Test Arrays: ");
	// println();
	// print((uint64_t*)"uint64_t a[3][3][3];");
	// println();
	// print((uint64_t*)"a[0][0][0] is initialize to 3");
	// println();
	// print((uint64_t*)"a[0][0][0] = 3 => it must be = ");
	// printInteger(a[0][0][0]);
	// println();
	// print((uint64_t*)"a[0][0][1] = 4 => it must be = ");
	// printInteger(a[0][0][1]);
	// println();
	// print((uint64_t*)"a[0][0][2] = 5 => it must be = ");
	// printInteger(a[0][0][2]);
	// println();
	// print((uint64_t*)"Matrix: ");
	// println();
	// i = 0;
	// while (i < 3) {
	//   j = 0;
	...
	//   i = i + 1;
	// }
	// UINT64_MAX = INT64_MIN - 1;
	// print((uint64_t*)"_INT64_MIN: ");
	// printInteger(INT64_MIN);
	// println();
	// print((uint64_t*)"UINT64_MAX:  ");
	// printInteger(UINT64_MAX);
	// println();
